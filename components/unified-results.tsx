...
interface UnifiedResultsProps {
  ...
  absResults: SearchResult[]
  webPagination?: PaginationInfo
  absPagination?: PaginationInfo
  isLoading: boolean
  onWebPageChange?: (page: number) => void
  onABSPageChange?: (page: number) => void
  activeTab: 'web' | 'abs'
  onTabChange: (tab: 'web' | 'abs') => void
}
...
export default function UnifiedResults({ 
  ...
  const hasABSResults = absResults.length > 0
  const hasAnyResults = hasWebResults || hasABSResults
      ...
      <div className="mt-6 text-center">
        <div className="max-w-md mx-auto">
          ...
          <p className="text-gray-500">
            Try adjusting your search query or check the available data sources.
          </p>
        </div>
      </div>
    )
    ...
    <div className="mt-6">
      ...
      <div className="mb-6">
        <p className="text-gray-600">
          {activeTab === 'web' ? (
            <>
              Found {webPagination?.totalResults || webResults.length} web result{(webPagination?.totalResults || webResults.length) !== 1 ? 's' : ''} from trusted sources
              {webPagination && webPagination.totalPages > 1 && (
                <span className="text-gray-500">
                  {' '}(Page {webPagination.currentPage} of {webPagination.totalPages})
                </span>
              )}
            </>
          ) : (
            <>
              Found {absPagination?.totalResults || absResults.length} ABS statistic{(absPagination?.totalResults || absResults.length) !== 1 ? 's' : ''} from official sources
              {absPagination && absPagination.totalPages > 1 && (
                <span className="text-gray-500">
                  {' '}(Page {absPagination.currentPage} of {absPagination.totalPages})
                </span>
              )}
            </>
          )}
      ...
      <div className="space-y-8">
        {activeTab === 'web' && hasWebResults && (
          <div>
            {webResults.map((result, index) => (
              <SearchResultCard key={index} result={result} />
            ))}
            
            {/* Web Results Pagination */}
            {webPagination && onWebPageChange && (
              <Pagination
                pagination={webPagination}
                onPageChange={onWebPageChange}
                isLoading={isLoading}
              />
            )}
          </div>
        ...
        {activeTab === 'abs' && hasABSResults && (
          <div>
            {absResults.map((result, index) => (
              <SearchResultCard key={index} result={result} />
            ))}
            
            {/* ABS Results Pagination */}
            {absPagination && onABSPageChange && (
              <Pagination
                pagination={absPagination}
                onPageChange={onABSPageChange}
                isLoading={isLoading}
              />
            )}
          </div>
        ...
        {activeTab === 'web' && !hasWebResults && hasABSResults && (
          <div className="text-center py-8">
            <Globe className="w-12 h-12 text-gray-400 mx-auto mb-4" />
            <h3 className="text-lg font-medium text-gray-900 mb-2">No web results found</h3>
            <p className="text-gray-500 mb-4">
              Try adjusting your search query or check the ABS Statistics tab for official data.
            </p>
            <button
              onClick={() => onTabChange('abs')}
        ...
        {activeTab === 'abs' && !hasABSResults && hasWebResults && (
          <div className="text-center py-8">
            <Database className="w-12 h-12 text-gray-400 mx-auto mb-4" />
            <h3 className="text-lg font-medium text-gray-900 mb-2">No ABS results found</h3>
            <p className="text-gray-500 mb-4">
              Try a different dataflow or check the Web Results tab for related content.
            </p>
            <button
              onClick={() => onTabChange('web')}
              className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors"
...
// Reuse the existing SearchResultCard component
function SearchResultCard({ result }: { result: SearchResult }) {
  const [isAnalyzing, setIsAnalyzing] = useReactState(false)
  const [showDataAsset, setShowDataAsset] = useReactState(false)
  const [isAnalyzingAI, setIsAnalyzingAI] = useReactState(false)
  const [showAIAnalysis, setShowAIAnalysis] = useReactState(false)
  const [aiAnalysis, setAiAnalysis] = useReactState<any>(null)
  const [dataAsset, setDataAsset] = useReactState(result.dataAsset)
  ...
  const handleAnalyzeContent = async () => {
    ...
    try {
      ...
      if (error instanceof Error && error.name === 'AbortError') { console.log('Dataset info fetch was cancelled'); return }
